...
export default function PersonalIntro({ 
  ...
  if (variant === 'compact') {
    return (
      <div className={getContainerClasses()}>
        ...
        {shouldShowTitle && (
          <small className="text-muted">{getTitleDisplay()}</small>
        )}
      </div>
    );
  }
  ...
  return (
    <div className={getContainerClasses()}>
      {shouldShowFullName && (
        <h1 className={`display-4 fw-bold mb-2 ${animateText ? 'animate-slide-down' : ''}`}>
          {getNameDisplay()}
        </h1>
      )}
      ...
      {shouldShowTagline && personalInfo.tagline && (
        <p className={`tagline text-muted mb-4 ${animateText ? 'animate-fade-in' : ''}`}
